...
public class Auth implements IAuth {
    ...
    public Auth(TokenService tokenService, Set<String> adminEmails) {
    ...
    @Override
    public Response<String> login(String username) { // login only for members!
    ...
    public Response<String> getRole(String token) {
        ...
        try {
            ...
            if (tokensLoggedOut.containsKey(token)) {
                logger.warning("Token belongs to a logged-out user (or a guest who left)");
                return new Response<>(null, "Token is logged out");
            }
    ...
    @Override
    public Response<Boolean> isLoggedIn(String token) { //for member
        if (token == null || token.isBlank()) {
            logger.warning("Token is null");
            return new Response<>(false, "Token is null");
        }
        try {
            if (tokenService.extractRole(token).equals("MEMBER") && !tokensLoggedOut.containsKey(token) && tokenService.validateToken(token)) {
